...
def profile_view(request):
    ...
    failed_mistakes_count = Exercise.objects.filter(
        attempts__user=user,
...
def missions_view(request):
    ...
    midnight = timezone.make_aware(midnight) if timezone.is_naive(midnight) else midnight
    ...
    course_ex_today = UserExerciseAttempt.objects.filter(
        user=user,
        attempted_at__range=(start_of_day, end_of_day),
        is_correct=True
    ).values('exercise').distinct().count()

    test_ex_today = TestAnswer.objects.filter(
        attempt__user=user,
    ...
    mistakes_percent = 100 if mistakes_completed else max(0, min(int(100 - (failed_mistakes_count / max(failed_mistakes_count, 1)) * 100), 99))
    ...
    missions = [
        ...
        },
        {
            ...
            'title': 'Чисто',
            'desc': f'Поверни і відпрацюй вправи, де помилився ({failed_mistakes_count} шт).' if not mistakes_completed else 'Усі помилки виправлено.',
            'current': f'{failed_mistakes_count} лишилось' if not mistakes_completed else 'Готово',
            'goal': '0 помилок',
            'current_label': '0 помилок 🎉' if mistakes_completed else f'{failed_mistakes_count} помилок',
