...
class GenerateExerciseView(APIView):
    ...
    def post(self, request):
        ...
        if not lesson_topic:
            return Response(
                {'error': 'Не вказано тему уроку'},
                status=status.HTTP_400_BAD_REQUEST
            )
        
        ai_service = AIExerciseService()
        exercise = ai_service.generate_exercise(lesson_topic, exercise_type, language, level)
...
class GetFeedbackView(APIView):
    ...
    def post(self, request):
        ...
        ai_service = AIExerciseService()
        feedback = ai_service.get_feedback(question, correct_answer, user_answer, exercise_type)
