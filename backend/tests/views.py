...
def tests_list_view(request):
    ...
    if request.user.is_authenticated:
        ...
        started_test_ids = set(
            TestAttempt.objects.filter(user=request.user, status="in_progress")
            .values_list("test_id", flat=True)
            .distinct()
        )
...
def test_detail_view(request, pk):
    ...
    if request.user.is_authenticated:
        ...
        completed_attempt = (
            TestAttempt.objects.filter(user=request.user, test=test, status="completed")
...
def test_attempt_view(request, pk):
    ...
    for section in sections:
        section_answers = TestAnswer.objects.filter(
            attempt=attempt, question__section=section
        )
        section_dict = {}
...
def submit_test_answer_view(request, attempt_id, question_id):
    ...
    question = get_object_or_404(
        TestQuestion, pk=question_id, section__test=attempt.test
    )
...
def complete_test_view(request, pk):
    ...
    for section in sections:
        answers = TestAnswer.objects.filter(attempt=attempt, question__section=section)
...
class SubmitAnswerView(APIView):
    ...
    def post(self, request, pk):
        ...
        if not question_id:
            return Response(
                {"error": "Не вказано ID питання"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class CompleteTestView(APIView):
    ...
    def post(self, request, pk):
        ...
        if attempt.status != "in_progress":
            return Response(
                {"error": "Тест вже завершено"}, status=status.HTTP_400_BAD_REQUEST
            )
