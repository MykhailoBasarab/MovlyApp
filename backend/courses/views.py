...
def mistakes_list_view(request):
    ...
    failed_exercises = Exercise.objects.filter(
        attempts__user=user,
        attempts__is_correct=False
    ).exclude(
        attempts__user=user,
        attempts__is_correct=True
    ).distinct().select_related('lesson', 'lesson__course', 'course_test', 'course_test__course').order_by('lesson__course', 'order')
    
    from django.urls import reverse
    
    # Останні спроби та джерела для кожної помилки
    for ex in failed_exercises:
        ex.last_attempt = UserExerciseAttempt.objects.filter(
            user=user,
            exercise=ex
        ...
        if ex.lesson:
            ...
            ex.source_url = reverse('courses:lesson-detail', kwargs={'pk': ex.lesson.id})
            ex.source_type = "УРОК"
        elif ex.course_test:
            ex.source_name = ex.course_test.course.title
            ex.source_url = reverse('courses:course-detail', kwargs={'pk': ex.course_test.course.id})
            ex.source_type = "ТЕСТ"
        else:
...
def courses_list_view(request):
    ...
    if request.user.is_authenticated:
        ...
        started_via_lessons = UserLessonProgress.objects.filter(
            user=request.user,
            lesson__course__in=courses
        ...
        started_via_exercises = UserExerciseAttempt.objects.filter(
            user=request.user,
            exercise__lesson__course__in=courses
        ).values_list('exercise__lesson__course_id', flat=True).distinct()
    ...
    if request.user.is_authenticated:
        for exercise in exercises:
            ...
            if attempt:
                ...
                last_attempt = UserExerciseAttempt.objects.filter(
                    user=request.user,
                    exercise=exercise
                ).order_by('-attempted_at').first()
                if last_attempt:
...
def submit_exercise_view(request, pk):
    ...
    if form.is_valid():
        user_answer = form.cleaned_data.get('answer') or form.cleaned_data.get('option', '')
        ai_service = None
        is_correct = False
        ...
        return redirect(reverse('courses:lesson-detail', kwargs={'pk': exercise.lesson.id}) + f'?feedback_ex={exercise.id}')
        ...
        if completed_lessons >= total_lessons:
            ...
            if not course_progress.is_completed:
                ...
                course_badge = Badge.objects.filter(
                    ...
                    criteria_value=lesson.course.id
                ).first()
                if course_badge:
        ...
        if not progress.is_completed:
            ...
            if completed_lessons >= total_lessons:
                ...
                # Нарахування XP за завершення курсу
                    ...
                    user_progress.award_badge('course_completed', criteria_value=lesson.course.id)
